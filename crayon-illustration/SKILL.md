...
When invoked, follow these 5 steps (Step 3 enters plan mode for multi-component layouts before the prompt is written):
...
### Step 2 — Type & Component Selection
...
### Step 3 — Plan Approval (plan mode) — conditional
...
For `concept-cards`, `mapping`, `grid`, and `architecture`/`workflow` with ≤ 4 components, skip this step and proceed directly to Step 4.

**When the gate fires,** compile a blueprint from Steps 1 + 2 and call `ExitPlanMode`:
...
On approval, proceed to Step 4. On rejection, ask which Step (1 or 2) needs revision, update it, and re-enter the gate.
...
4. Generate the full prompt text using all discovery + component decisions from Steps 1–2
